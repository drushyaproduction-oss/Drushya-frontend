...
import { getAllBookingRequests, updateBookingRequest, deleteBookingRequest, sendBookingEmailApi } from '../../../api/booking';
...
const Bookings = () => {
  ...
  const handleSave = async () => {
    if (modalMode === 'edit' && selectedBooking) {
      try {
        const res = await updateBookingRequest(selectedBooking._id, formData);
        if (res && res.success) {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await updateBookingRequest(id, { status: newStatus });
      if (res && res.success) {
  ...
  const handleSendEmail = async (id) => {
    try {
      const res = await sendBookingEmailApi(id);
      if (res && res.success) {
      ...
      <div className="flex-1 lg:ml-72 flex flex-col min-w-0 transition-all duration-300">
        ...
        <div className="p-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-visible">
            <div className="overflow-x-auto min-h-[250px] pb-8">
                    ...
                    <tr key={booking._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-4 px-6 text-right">
                        <ActionMenu 
                          ...
                          onSendEmail={booking.status === 'Confirmed' ? () => handleSendEmail(booking._id) : undefined}
          ...
          {selectedBooking && (
            <div className="flex-1 overflow-y-auto pr-2">
              <div className="bg-gray-50 p-4 rounded-xl mb-6">
                ...
                <div className="font-bold text-gray-900 text-lg mb-2">{selectedBooking.customerName}</div>
                <div className="flex flex-col gap-1 text-sm text-gray-600">
                  <div><strong>Email:</strong> {selectedBooking.email}</div>
                  <div><strong>Phone:</strong> {selectedBooking.mobile}</div>
                </div>
              </div>
              ...
              <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-xl mb-6">
                ...
                <div className="flex flex-col gap-2 text-sm text-gray-700">
                  <div><strong>Package:</strong> {selectedBooking.packageId?.title || 'Unknown Package'}</div>
                  <div><strong>Studio:</strong> {selectedBooking.studioLocation || <span className="text-gray-400">-</span>}</div>
                  <div><strong>Date:</strong> {new Date(selectedBooking.eventDate).toLocaleDateString()}</div>
                  <div><strong>Location:</strong> {selectedBooking.eventLocation}</div>
                  {selectedBooking.message && <div><strong>Message:</strong> {selectedBooking.message}</div>}
                </div>
              </div>
